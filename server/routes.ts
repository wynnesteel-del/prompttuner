...
AI target: ${aiTarget || "perplexity"}`;
  ...
  // Generate role-based prompt suggestions using AI (unlimited roles)
  app.post("/api/generate-role-prompts", async (req, res) => {
    try {
      const { role } = req.body;
      ...
      const response = await anthropic.messages.create({
        model: "claude-sonnet-4-6",
        max_tokens: 1024,
        system: `You are a prompt discovery engine for PromptTuner. Given any professional role, expert, or persona, generate exactly 8 highly specific, genuinely useful prompts that someone would ask an AI playing that role. The prompts should be practical, immediately useful in real life, cover a range of the role's most valuable use cases, and feel like things real people actually need to know. Be varied - don't repeat similar topics. Return ONLY a JSON array of exactly 8 strings. No markdown, no explanation, no code blocks. Just the raw JSON array starting with [ and ending with ].`,
        messages: [{ role: "user", content: `Generate 8 prompt suggestions for the role: ${roleName}` }],
      });
      const textContent = response.content.find((c) => c.type === "text");
      if (!textContent || textContent.type !== "text") {
        throw new Error("No response from AI");
      }
      const prompts = parseJsonFromResponse(textContent.text);
      if (!Array.isArray(prompts)) {
        throw new Error("Invalid response format");
      }
      res.json({ role: roleName, prompts });
    } catch (error: any) {
      console.error("Generate role prompts error:", error);
      res.status(500).json({ message: error.message });
    }
  });
